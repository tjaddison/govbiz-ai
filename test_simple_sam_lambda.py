#!/usr/bin/env python3
"""
Test the simple SAM downloader Lambda function
"""
...
def test_simple_sam_lambda():
    ...
    print("=" * 60)
    print("TESTING SIMPLE SAM CSV DOWNLOADER")
    print("=" * 60)
    ...
    try:
        ...
        if response['StatusCode'] == 200:
            ...
            if 'body' in response_payload:
                ...
                if body.get('success'):
                    print("✓ SAM CSV download and processing completed successfully")
                    
                    # Show processing stats
                    data = body.get('data', {})
                    if data:
                        print(f"  - Total opportunities processed: {data.get('total_processed', 0)}")
                        print(f"  - Opportunities inserted: {data.get('inserted', 0)}")
                        print(f"  - Opportunities updated: {data.get('updated', 0)}")
                        print(f"  - Errors: {data.get('errors', 0)}")
                        print(f"  - Processing time: {data.get('processing_time_seconds', 0):.2f} seconds")
                else:
                    print(f"✗ SAM CSV processing failed: {body.get('error', 'Unknown error')}")
            else:
    ...
    # Test 3: Check EventBridge rule configuration
    print("\n3. Checking EventBridge rule configuration...")
            ...
            if targets:
                ...
                for target in targets:
                    ...
                    if lambda_function_name in target.get('Arn', ''):
                        print(f"    - ✓ Points to simple SAM downloader")
                    else:
                        print(f"    - ⚠ Points to different function")
            else:
    ...
    print("TEST SUMMARY")
    print("=" * 60)
    print("✅ Simple SAM CSV downloader test completed successfully!")
...
if __name__ == "__main__":
    success = test_simple_sam_lambda()
